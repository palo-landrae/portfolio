...
export const Navbar = () => {
    ...
    return (
        <nav className='flex items-center justify-between flex-wrap p-3 dark:bg-metal bg-dawn-nav dark:text-white text-dawn-text overflow-x-clip'>
            <div className='mx-auto flex inline-flex max-w-screen-lg'>
                <Link href='/'>
                    <a className='inline-flex items-center p-2 mr-4 md:mr-20'>
                ...
                <div className='hidden md:block md:inline-flex md:flex-row md:ml-auto md:w-auto w-full md:items-center items-start flex flex-col md:h-auto'>
                    {menuData.map((menu) => (
                        <Link key={menu.href} href={menu.href}>
                            <a
                                className={`${
                                    router.pathname === menu.href ? "underline underline-offset-4" : ""
